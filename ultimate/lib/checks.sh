...
# ---------------------------------------------- extra checkers for this series
# file_has <desc> <path> <substring>   — grep a file on the node's filesystem
file_has() { local d="$1" f="$2" want="$3"
  if [ -f "$f" ] && grep -qF -- "$want" "$f"; then ok "$d"
  else no "$d  ${DIM}('$want' not found in $f)${NC}"; fi; }

# file_matches <desc> <path> <extended regex>
file_matches() { local d="$1" f="$2" re="$3"
  if [ -f "$f" ] && grep -qE -- "$re" "$f"; then ok "$d"
  else no "$d  ${DIM}(no line matching /$re/ in $f)${NC}"; fi; }

# file_nonempty <desc> <path>
file_nonempty() { local d="$1" f="$2"
  if [ -s "$f" ]; then ok "$d"; else no "$d  ${DIM}($f missing or empty)${NC}"; fi; }

# absent <desc> <kubectl get args...>  — passes when the object does NOT exist
absent() { local d="$1"; shift
  if kubectl get "$@" >/dev/null 2>&1; then no "$d  ${DIM}(still exists)${NC}"; else ok "$d"; fi; }

# ready <desc> <namespace> <deployment>  — readyReplicas equals spec.replicas
ready() { local d="$1" ns="$2" dep="$3" want got
  want="$(kubectl get deploy "$dep" -n "$ns" -o jsonpath='{.spec.replicas}' 2>/dev/null)"
  got="$(kubectl get deploy "$dep" -n "$ns" -o jsonpath='{.status.readyReplicas}' 2>/dev/null)"
  if [ -n "$want" ] && [ "$want" = "$got" ]; then ok "$d"
  else no "$d  ${DIM}(ready ${got:-0}/${want:-?})${NC}"; fi; }

# pod_running <desc> <namespace> <pod>
pod_running() { local d="$1" ns="$2" pod="$3" got
  got="$(kubectl get pod "$pod" -n "$ns" -o jsonpath='{.status.phase}' 2>/dev/null)"
  if [ "$got" = "Running" ]; then ok "$d"; else no "$d  ${DIM}(phase ${got:-<none>})${NC}"; fi; }

# endpoints_at_least <desc> <namespace> <service> <n>
endpoints_at_least() { local d="$1" ns="$2" svc="$3" n="$4" got
  got="$(kubectl get endpoints "$svc" -n "$ns" -o jsonpath='{.subsets[*].addresses[*].ip}' 2>/dev/null | wc -w)"
  if [ "${got:-0}" -ge "$n" ]; then ok "$d"; else no "$d  ${DIM}($got endpoint(s), want >= $n)${NC}"; fi; }
