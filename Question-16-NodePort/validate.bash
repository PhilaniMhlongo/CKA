...
# 12. Headless service nodeport-headless exists
check "Service 'nodeport-headless' exists in namespace '$NS'" \
  kubectl get svc nodeport-headless -n "$NS"

# 13. Headless service has clusterIP None (this is what exposes individual pods)
check "Service 'nodeport-headless' is headless (clusterIP: None)" \
  bash -c '[[ "$(kubectl get svc nodeport-headless -n '"$NS"' -o jsonpath="{.spec.clusterIP}")" == "None" ]]'

# 14. Headless service port/protocol
check "Headless service port is 80 and protocol is TCP" \
  bash -c '
    PORT=$(kubectl get svc nodeport-headless -n '"$NS"' -o jsonpath="{.spec.ports[0].port}" 2>/dev/null)
    PROTO=$(kubectl get svc nodeport-headless -n '"$NS"' -o jsonpath="{.spec.ports[0].protocol}" 2>/dev/null)
    [[ "$PORT" == "80" && "$PROTO" == "TCP" ]]
  '

# 15. Headless service resolves to every pod, not one virtual IP
check "Headless service has one endpoint address per running pod" \
  bash -c '
    PODS=$(kubectl get pods -n '"$NS"' -l app=nodeport-deployment --field-selector=status.phase=Running --no-headers 2>/dev/null | wc -l)
    ADDRS=$(kubectl get endpoints nodeport-headless -n '"$NS"' -o jsonpath="{.subsets[0].addresses[*].ip}" 2>/dev/null | wc -w)
    [[ $PODS -gt 0 && $ADDRS -eq $PODS ]]
  '
